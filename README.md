...
📘 SOC Detection Lab — Docker-Based Home Lab
🔍 Overview
...
🔹 Log parsing & centralized log collection

🔹 Building detection rules based on log patterns

🔹 Identifying indicators of scanning & enumeration

🔹 Writing and exposing detections via API endpoints

🔹 MITRE ATT&CK mapping to real techniques
...
log-analyzer-api → http://localhost:4100

target-web → accessible via Docker network

attacker → interactive shell
...
⚔️ Simulating Attacks (from attacker container)
...
Web/Port Scan	T1046 – Network Service Scanning	The attacker probes open ports and services using Nmap
Active Enumeration	T1595 – Active Scanning	High-volume HTTP requests simulate reconnaissance
📂 Project Structure
soc-detection-lab/
│── app/
│   ├── app.py              # Python mini-SIEM / detection engine
│   └── requirements.txt
│── logs/                   # (unused placeholder)
│── nginx_logs/             # Shared log volume from target-web
│── docker-compose.yml
...
✅ Working MVP
➡️ Next Step: Incident Report + Additional Detection
